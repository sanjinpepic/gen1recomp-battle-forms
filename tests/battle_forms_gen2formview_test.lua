...
for _, name in ipairs({ "src.ui.gen2.SummaryMenu", "src.ui.gen2.Chrome",
    "src.render.GbcPalette", "src.world.gen2.Palettes",
    "src.ui.gen2.PartyMenu", "src.render.Assets" }) do
...
-- ---------------------------------------------------------------------
-- PartyMenu's list icon: the same species-keyed gap SummaryMenu's picture
-- had, fixed the same way -- fall back to the form record's own spriteFront
-- (or a real pokemon.sprite hook's own answer), fitted into the icon's
-- 16x16 slot instead of the SUMMARY picture's 56x56 one.  M.drawIcon is
-- exercised directly first, with no require/stub involved at all -- it
-- takes its `self` as a plain argument, so its own correctness needs
-- nothing more than a fixture table shaped like the fields it reads.
-- ---------------------------------------------------------------------
Gen2FormView.bind({ fusion = Fusion, persistent = Persistent })

-- A resolveModule that always answers nil (no sprite mod loaded), so the
-- record fallback (formDef.spriteFront) is what these prove -- the half of
-- the fix that needs no neighbour to work, exactly the SUMMARY picture
-- section's own first pass above.
local function noHook() return nil end

local function iconFixture(mon)
  local drawn = {}
  local realDraw = love.graphics.draw
  local self = {
    game = { data = DATA },
    palettes = nil,
    -- Real methods, via the real class's own metatable -- heldMarkerRow and
    -- heldMarkerImage are stateless aside from self.icons/self.iconCache,
    -- and leaving those nil is the honest "no held-item art loaded" case
    -- rather than a stub reimplementing the real logic.
  }
  local RealPartyMenu = require("src.ui.gen2.PartyMenu")
  setmetatable(self, RealPartyMenu)
  self.icons, self.iconCache = nil, {}
  love.graphics.draw = function(image, ...)
    drawn[#drawn + 1] = image
    return realDraw(image, ...)
  end
  local ok, drew = pcall(Gen2FormView.drawIcon, self, mon, 0, 0, noHook)
  love.graphics.draw = realDraw
  return ok, drew, drawn, self
end

do
  local plain = { species = "PIDGEY", level = 5, dvs = {}, statExp = {} }
  local ok, drew = iconFixture(plain)
  T.check(ok, "an unformed mon does not throw")
  T.eq(drew, false, "and returns false -- the caller falls back to vanilla drawIcon")
end

do
  local rotom = persistentMon("ROTOM", "WASHING_MACHINE")
  local ok, drew, drawn = iconFixture(rotom)
  T.check(ok, "a formed mon does not throw")
  T.eq(drew, true, "and returns true -- vanilla drawIcon must not also run")
  local sawFormIcon, sawBaseIcon = false, false
  for _, image in ipairs(drawn) do
    if image and image.path == DATA.pokemon.ROTOM_WASH.spriteFront then sawFormIcon = true end
    if image and image.path == DATA.pokemon.ROTOM.spriteFront then sawBaseIcon = true end
  end
  T.check(sawFormIcon, "the list icon drew Rotom-Wash's OWN picture, fitted "
...
-- M.drawIcon's identical per-instance cache: two DIFFERENT forms of the
-- SAME base species must not answer each other's row.
do
  ...
  local realDraw = love.graphics.draw
  local RealPartyMenu = require("src.ui.gen2.PartyMenu")
  local self = setmetatable({ game = { data = DATA }, icons = nil, iconCache = {} },
    RealPartyMenu)
  love.graphics.draw = function(image, ...)
    drawn[#drawn + 1] = image
    return realDraw(image, ...)
  end
  local okWash = pcall(Gen2FormView.drawIcon, self, wash, 0, 0, noHook)
  local okHeat = pcall(Gen2FormView.drawIcon, self, heat, 16, 0, noHook)
  love.graphics.draw = realDraw
  T.check(okWash and okHeat, "both draws run without error")
  local sawWash, sawHeat = false, false
  for _, image in ipairs(drawn) do
    if image and image.path == DATA.pokemon.ROTOM_WASH.spriteFront then sawWash = true end
    if image and image.path == DATA.pokemon.ROTOM_HEAT.spriteFront then sawHeat = true end
  end
  T.check(sawWash, "the first Rotom's row drew its OWN form's icon")
  T.check(sawHeat, "the second Rotom's row drew ITS OWN form's icon -- not "
    .. "the first one's, even though both cache under the same instance "
    .. "(formId, not species, is the cache key)")
end

-- The held-item marker: a formed mon is, by construction, holding the item
-- that grants the form, so drawing the form's own picture instead of
-- vanilla's icon sheet must not silently drop the marker vanilla drawIcon
-- would have drawn for it.
do
  local rotom = persistentMon("ROTOM", "WASHING_MACHINE")
  -- PartyMenu.heldMarkerRow reads mon.item, the real Gen 2 held-item slot --
  -- a DIFFERENT field from Eligibility.STAMP, which is what persistentMon's
  -- own mark() call stamps.  A real Rotom carries both: the stamp is how
  -- src/persistent.lua derives the form, item is what HeldItemMenu's GIVE
  -- actually writes and what PartyMenu's own marker reads.
  rotom.item = "WASHING_MACHINE"
  local RealPartyMenu = require("src.ui.gen2.PartyMenu")
  local markerImage = { path = "fake-held-marker", getDimensions = function() return 16, 8 end }
  local self = setmetatable({
    game = { data = DATA }, iconCache = {},
    icons = { heldItem = { image = "marker.png" } },
  ...
  love.graphics.draw = realDraw
  T.check(ok and drew, "drawing a formed, item-holding mon's icon succeeds")
  local sawMarker = false
  for _, image in ipairs(drawn) do
    if image == markerImage then sawMarker = true end
  end
  T.check(sawMarker, "and the held-item marker still drew, on top of the "
    .. "form's own picture rather than vanilla's icon sheet")
end
...
do
  ...
  local RealPartyMenu = require("src.ui.gen2.PartyMenu")
  local RealSummaryMenu2 = require("src.ui.gen2.SummaryMenu")
  T.eq(Gen2FormView.install(fakeMod()), true,
    "install succeeds against the real PartyMenu class too")
  T.check(RealPartyMenu._battleFormsGen2FormViewIcon,
    "PartyMenu.drawIcon carries the installed marker")

  local vanillaCalls = 0
  local vanillaDrawIcon = RealPartyMenu.drawIcon
  -- Swap AFTER install so this counts vanilla calls through the wrapper,
  -- exactly the way the SummaryMenu drawPic composition tests above do.
  local spied = function(self, mon, px, py)
    vanillaCalls = vanillaCalls + 1
    return vanillaDrawIcon(self, mon, px, py)
  end
  -- The installed wrapper closed over the ORIGINAL vanillaDrawIcon at
  -- install time, so spying requires reinstalling is not an option (a
  -- second install is a guarded no-op) -- instead this drives the wrapper
  -- that is already in place and asks whether the base species' own picture
  -- (proof vanilla ran) or the form's (proof this module's own draw ran)
  -- came out, the same "spy on what was drawn" technique the SUMMARY tests
  -- above already use rather than counting calls through a swapped vanilla.
  local self = setmetatable({ game = { data = DATA }, icons = nil, iconCache = {} },
    RealPartyMenu)
  ...
  local rotom = persistentMon("ROTOM", "WASHING_MACHINE")
  local okFormed = pcall(RealPartyMenu.drawIcon, self, rotom, 0, 0)
  love.graphics.draw = realDraw
  T.check(okFormed, "the wrapped PartyMenu.drawIcon runs on a formed mon without error")
  local sawFormIcon = false
  for _, image in ipairs(drawn) do
    if image and image.path == DATA.pokemon.ROTOM_WASH.spriteFront then sawFormIcon = true end
  end
  T.check(sawFormIcon,
    "and the real, installed wrapper draws the form's own picture -- proof "
      .. "the install call actually reaches the live class, not only a stub")
end
